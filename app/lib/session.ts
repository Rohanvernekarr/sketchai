...
export async function createAnonymousSession(ipAddress?: string, userAgent?: string) {
...
export async function createSessionRecord(sessionId: string, ipAddress?: string, userAgent?: string) {
...
export async function checkUserLimits(sessionId?: string, userId?: string) {
  ...
  if (userId) {
    ...
    const dailyLimit = 3
    const currentCount = dailyUsage.needsReset ? 0 : dailyUsage.dailyPromptCount
