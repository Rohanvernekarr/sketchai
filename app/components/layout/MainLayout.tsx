...
export default function MainLayout() {
  ...
  const handleRedo = useCallback(() => {
    if (historyIndex < history.length - 1) {
      ...
      //re-render automatically due to systemElements/connections props change
    }
  }, [historyIndex, history, currentDiagram]);
    ...
    saveTimeoutRef.current = setTimeout(() => {
      ...
      if (
        ...
        JSON.stringify(currentState.elements) ===
          JSON.stringify(newState.elements) &&
  ...
  return (
    <div className="h-screen w-full flex flex-row select-none overflow-hidden">
      <div className="flex-1 flex flex-row relative">
          ...
          <div className="absolute bottom-2 left-2 sm:bottom-4 sm:left-4 z-10 max-w-[95%] sm:max-w-none">
            <div className="bg-black border border-white rounded p-1.5 sm:p-2 text-white text-[10px] sm:text-xs">
              <div className="hidden sm:block">
                Elements: {systemElements.length} | Connections:{" "}
                {connections.length} | Strokes: {freehandStrokes.length} |
        ...
        <RightToolbar activeTool={activeTool} onToolChange={setActiveTool} />
      </div>
