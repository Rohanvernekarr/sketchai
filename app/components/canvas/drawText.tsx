...
function drawCursor(ctx: CanvasRenderingContext2D, textElement: TextElement, cursorPosition: number) {
  const lines = textElement.text.split('\n');
  let currentPos = 0;
  let cursorLine = 0;
  let cursorColumn = 0;
  ...
  const cursorY = textElement.position.y + (cursorLine * textElement.fontSize * 2.2);
